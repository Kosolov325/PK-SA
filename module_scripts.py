...
scripts.extend([
  ...
  ("show_factions_rent_levels", 
  [(store_script_param_1, ":player_id"),
   (multiplayer_send_string_to_player, ":player_id", server_event_script_message, "@Factions rent Levels:"),
   (try_for_range, ":faction_id", castle_factions_begin, factions_end),
    (faction_get_slot, reg0, ":faction_id", slot_faction_rent),
    (str_store_faction_name, s0, ":faction_id"),
    (multiplayer_send_string_to_player, ":player_id", server_event_script_message, "@{s0} - Rent: {reg0}"),
   (try_end),
  ]),

  ("handle_chat_commands",
   [(store_script_param_1, ":sender_player_id"),
    (store_script_param_2, s0),
    
    (str_store_substring, s0, s0, 1),
    (assign, ":failure", 0),
    (try_begin),
      (str_equals, s0, "@help"),
      (multiplayer_send_string_to_player, ":sender_player_id", server_event_script_message, "@List of commands:"),
      (multiplayer_send_string_to_player, ":sender_player_id", server_event_script_message, "@/showrents - Show faction's rent levels."),
    (else_try),
      (str_equals, s0, "@showrents"),
      (call_script, "script_show_factions_rent_levels", ":sender_player_id"),
    (else_try),
      (assign, ":failure", 1),
    (try_end),
    ...
    (try_begin),
     (eq, ":failure", 1),
     (multiplayer_send_string_to_player,":sender_player_id", server_event_script_message, "@This command doesnt exist!"),
     (multiplayer_send_string_to_player,":sender_player_id", server_event_script_message, "@Try /help"),
     (multiplayer_send_int_to_player, ":sender_player_id", server_event_play_sound, "snd_failure"),
    (try_end),
   ]),
      ...
      (else_try), # chat string received from a client, from a range of event numbers to keep them in order
        ...
        (try_begin),
          ...
          (try_begin),
            ...
            (try_begin),
              (str_starts_with, s0, "@/"),
              (call_script, "script_handle_chat_commands", ":sender_player_id", s0),
            (else_try),
  ...
  ("initialize_scene_globals", # reset global values to initial values when starting a mission
   [
    ...
    (try_for_range, ":faction_id", factions_begin, factions_end),
      ...
      (try_begin),
        (is_between, ":faction_id", castle_factions_begin, factions_end),
        (faction_set_slot, ":faction_id", slot_faction_rent, 1),
      (try_end),
  ...
  ("calculate_faction_rent_price",
   [(store_script_param, ":player_id", 1),
    ...
    (try_begin),
      ...
      (store_mul, ":rent_amount", ":value", ":rent_percentage"),
      (val_div, ":rent_amount", 100),
      (assign, reg1, ":rent_amount"),
      (multiplayer_send_string_to_player, ":player_id", server_event_script_message, "@You received {reg1} more gold from your faction rent level."),
    (try_end),
    (val_add, ":value", ":rent_amount"),
    (assign, reg0, ":value"),
  ]),

  ("decrease_and_increase_faction_rent", 
   [(store_script_param, ":loser_faction_id", 1),
    (store_script_param, ":winner_faction_id", 2),

    # Decrease loser faction's rent value (except fac_commoners)
    (try_begin),
      (neq, ":loser_faction_id", "fac_commoners"),
      (faction_get_slot, ":loser_rent_value", ":loser_faction_id", slot_faction_rent),
      (val_sub, ":loser_rent_value", 1),
      (val_max, ":loser_rent_value", 0),  # Limit to 0 or above
      (faction_set_slot, ":loser_faction_id", slot_faction_rent, ":loser_rent_value"),
    (try_end),

    # Increase winner faction's rent value
    (faction_get_slot, ":winner_rent_value", ":winner_faction_id", slot_faction_rent),
    (val_add, ":winner_rent_value", 1),
    (faction_set_slot, ":winner_faction_id", slot_faction_rent, ":winner_rent_value"),
    ...
    (try_for_players, ":player_id"),
      ...
      (else_try),
        (neq, ":faction_id", "fac_commoners"),
        (eq, ":faction_id", ":loser_faction_id"), 
        (assign, reg2, ":loser_rent_value"),
        (multiplayer_send_string_to_player, ":player_id", server_event_script_message_announce, "@Your faction rent level has decreased. Level: {reg2}"),
